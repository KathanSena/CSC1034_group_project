...
let stockFilterChart = null;

const drawStockFilterChart = (rows) => {
    const canvas = document.querySelector("#stockFilterChart");
    ...
    for (let row of rows) {
        ...
        categoryTotals[category] = (categoryTotals[category] || 0) + Number(row.quantity_in_stock);
    }
...
const loadStockItems = async () => {
    const select = document.querySelector("#stockSearch");
    const sql = "SELECT item_name FROM FoodItem ORDER BY item_name;";
    const result = await runQuery(sql);
...
const printTable = async () => {
    ...
    const searchText = document.querySelector("#stockSearch").value;
    ...
    if (searchText !== "") {
        ...
        sql += ` AND fi.item_name = '${safeSearch}'`;
    }
